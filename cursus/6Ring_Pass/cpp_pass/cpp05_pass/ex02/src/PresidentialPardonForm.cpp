...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/07 20:37:59 by ingonzal          #+#    #+#             */
/*   Updated: 2023/05/07 21:06:16 by ingonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "PresidentialPardonForm.hpp"

PresidentialPardonForm::PresidentialPardonForm(void) :  AForm("Basic PresidentialPardonForm", 25, 5, false), _target("Basic target"){
	std::cout << *this << " Applied" << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(std::string target) : AForm("PresidentialForm", 25, 5, false), _target(target){
		std::cout << *this << " Applied" << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(PresidentialPardonForm const & src) : AForm(src.getName() + " Photocopy", src.getWgrade(), src.getXgrade(), src.getSign()), _target(src.getTarget()){
